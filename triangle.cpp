...
{
	pointlist = NULL;                                               /* In / out */
	pointattributelist = NULL;                                      /* In / out */
	pointmarkerlist = NULL;                                          /* In / out */
	numberofpoints = 0;                                            /* In / out */
	numberofpointattributes = 0;                                   /* In / out */

	trianglelist = NULL;                                             /* In / out */
	triangleattributelist = NULL;                                   /* In / out */
	trianglearealist = NULL;                                         /* In only */
	neighborlist = NULL;                                             /* Out only */
	numberoftriangles = 0;                                         /* In / out */
	numberofcorners = 0;                                           /* In / out */
	numberoftriangleattributes = 0;                                /* In / out */

	segmentlist = NULL;                                              /* In / out */
	segmentmarkerlist = NULL;                                        /* In / out */
	numberofsegments = 0;                                          /* In / out */

	holelist = NULL;                        /* In / pointer to array copied out */
	numberofholes = 0;                                      /* In / copied out */

	regionlist = NULL;                      /* In / pointer to array copied out */
	numberofregions = 0;                                    /* In / copied out */

	edgelist = NULL;                                                 /* Out only */
	edgemarkerlist = NULL;            /* Not used with Voronoi diagram; out only */
	normlist = NULL;                /* Used only with Voronoi diagram; out only */
	numberofedges = 0;
}

triangulateio::~triangulateio()
{
	free(pointlist);                                               /* In / out */
	free(pointattributelist);                                      /* In / out */
	free(pointmarkerlist);                                          /* In / out */
	free(trianglelist);                                             /* In / out */
	free(triangleattributelist);                                   /* In / out */
	free(trianglearealist);                                         /* In only */
	free(neighborlist);                                             /* Out only */
	free(segmentlist);                                              /* In / out */
	free(segmentmarkerlist);                                        /* In / out */
	free(holelist);                        /* In / pointer to array copied out */
	free(regionlist);                      /* In / pointer to array copied out */
	free(edgelist);                                                 /* Out only */
	free(edgemarkerlist);            /* Not used with Voronoi diagram; out only */
	free(normlist);                /* Used only with Voronoi diagram; out only */
}
...
int triunsuitable(vertex triorg, vertex tridest, vertex triapex, REAL area)
// triorg;                              /* The triangle's origin vertex. */
// tridest;                        /* The triangle's destination vertex. */
// triapex;                               /* The triangle's apex vertex. */
