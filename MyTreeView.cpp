...
void CMyTreeView::MoveSelectedPoint()
{
	...
	{
		int flag = pMesh->pmesh->property(vprop_selectedflag,v_it.handle());
		if(flag)
		{
			vh = v_it.handle(); 
			selectnum++;
	...
	GLfloat zWin;
	Point3D proPoint;
	yWin = pMesh->m_viewPort[3] - (float)yWin - 1;
	glReadPixels((int)xWin, (int)yWin, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &zWin); 
	GLdouble x,y,z;
	OpenMesh::Vec3d vec = pMesh->pmesh->point(vh);
	gluUnProject(xWin,yWin,zWin,pMesh->modelview,pMesh->projection,pMesh->m_viewPort,&(x),&(y),&(z));
	OpenMesh::Vec3d pt;
	pt[0] = x;
	pt[1] = y;
	pt[2] = 0;
	pMesh->pmesh->set_point(vh,pt);

	sprintf(Post_Message,"Vertex (%d). Coordinates: (%f %f %f)",vh.idx(),pt[0],pt[1],pt[2]);
